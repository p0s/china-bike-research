...
Complete-bike research and display are trim-specific. Each exact build should seek a complete weight in kilograms with its size and equipment basis when available, the manufacturer's maximum tire clearance or a clearly distinct fitted-tire observation, its exact drivetrain, a dated price for that same selected build, and all supporting sources. Never assemble a favorable row from the lightest weight, best drivetrain, and lowest price of different trims. When one model has several documented builds, the catalog may use one clearly identified preferred reference trim while its model page lists the alternative build-specific weights, drivetrains, and prices. Framesets instead prioritize frameset-package weight and basis, maximum tire clearance, dated price, and build-critical standards.
...
- Third-party images may be remotely embedded or served from the approved media origin only with source, owner or seller credit, exactness, rights status, alt text, privacy review where applicable, and documented failure behavior.
...
- Public XHS and Taobao source links must be canonical and identity-safe. Strip share, referral, invite, tracking, session, and account parameters. Publish an XHS `/explore/<post-id>` URL without query parameters or a Taobao item URL containing only the public `id`; if no stable identity-safe URL exists, retain the unsafe URL only in local evidence. A non-image fact may cite the platform, public item/post ID, and source label without a link, but the image and unsafe URL stay unpublished.

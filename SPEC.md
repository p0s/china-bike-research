...
- `public-post-quotation` normally uses the approved project media origin. When that route is unavailable or a durable specification quotation is materially useful, `source-attributed-rehost` may store the same bounded derivatives under `assets/images/sourced/xhs/` or `assets/images/sourced/taobao/`. Raw captures never enter Git. The public repository contains only exact-model, decision-relevant WebP derivatives plus their source and review metadata.

...
function priceCell(ctx, price) {
  const condition = price?.conditions || price?.price_basis;
  return `${escapeHtml(formatPrice(price))}${price?.observed_at ? `<small><time datetime="${escapeAttr(price.observed_at)}">${escapeHtml(price.observed_at)}</time></small>` : ''}${condition ? `<details class="price-conditions"><summary>${t(ctx, 'Price details', '价格详情')}</summary><p lang="en" data-original-language>${escapeHtml(condition)}</p>${ctx.locale === 'zh-Hans' ? '<small>原始价格说明（英文）</small>' : ''}</details>` : ''}`;
}
...
export function renderEvidenceTable(ctx, post) {
  ...
  if (kind === 'gravel') {
    const rows = models.map((model) => {
      ...
      const weightCell = weight ? `${(weight / 1000).toLocaleString('en-US')} kg<details class="price-conditions"><summary>${labels('Weighing basis', '称重口径')}</summary><p lang="en" data-original-language>${escapeHtml(variant.claimed_complete_weight_basis ?? labels('Basis not recorded', '未记录口径'))}</p></details>` : '—';
      const max = model.product.platform.tire_clearance?.published_max_mm;
      const clearance = max ? labels(`Up to ${max} mm`, `最高 ${max} mm`) : '—';
      return `<tr>${modelCell(ctx, model)}<td>${priceCell(ctx, model.price)}<small>${note(model)}</small></td><td>${escapeHtml(drive ? `${drive.brand} ${drive.model}` : '—')}<small>${shifting} · ${escapeHtml(drive?.speeds?.replace('x', '×') ?? '—')}</small></td><td>${clearance}</td><td>${weightCell}</td></tr>`;
    });
  ...
  if (kind === 'price-basis') {
    const rows = models.map((model) => `<tr>${modelCell(ctx, model)}<td>${model.kind === 'frameset' ? labels('Frame package', '车架套餐') : labels('Complete bike', '整车')}</td><td>${priceCell(ctx, model.price)}</td><td>${note(model)}</td></tr>`);
...
export function renderBuildExample(ctx) {
  ...
  const rows = [
    `<tr><th scope="row"><a href="${url(ctx.base, '/models/incolor-voyager-frameset/')}">INCOLOR Voyager</a></th><td>${priceCell(ctx, frame.price)}</td><td>${label('Official starting price; confirm the selected cockpit option.', '官方起价；需确认所选套餐是否含把组。')}</td></tr>`,

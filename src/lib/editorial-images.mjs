...
export function editorialImageMeta(ctx, id) {
  const photo = resolveBlogPhoto(ctx, editorialImage(id).photo_id);
  return { image: photo.image.hosting.remote_url, imageAlt: photo.alt[ctx.locale ?? 'en'] };
}
...
function renderPhoto(ctx, photo, image, { card = false, banner = false, inline = false, modelIds = [], href = '' } = {}) {
  const locale = ctx.locale ?? 'en';
  const zh = locale === 'zh-Hans';
  const classes = inline ? 'blog-model-photo' : card ? 'article-card-image' : banner ? 'blog-banner' : 'article-cover';
  ...
  return `<figure class="editorial-figure ${classes}" data-blog-photo="${photo.id}">${href ? `<a class="blog-photo-scene" href="${escapeAttr(href)}" tabindex="-1" aria-hidden="true">` : '<div class="blog-photo-scene">'}<img class="blog-bike-photo" data-blog-bike-image src="${escapeAttr(photo.image.hosting.remote_url)}" alt="${escapeAttr(photo.alt[locale])}" referrerpolicy="no-referrer" loading="${card || inline ? 'lazy' : 'eager'}" decoding="async"${card || inline ? '' : ' fetchpriority="high"'}>${mascot(ctx, image)}${href ? '</a>' : '</div>'}<figcaption><strong>${escapeHtml(photo.name)}</strong>${links ? ` · ${links}` : ''}<span class="blog-photo-note">${escapeHtml(photo.note[locale])}</span><a href="${escapeAttr(photo.source.url)}" rel="noreferrer">${escapeHtml(photo.image.credit)}</a><span class="blog-photo-status" data-blog-photo-status hidden>${zh ? '照片暂时无法加载，可打开来源查看。' : 'Photo unavailable; open the source to view it.'}</span></figcaption></figure>`;
}

export function renderEditorialImage(ctx, id, options = {}) {
  const image = editorialImage(id);
  return renderPhoto(ctx, resolveBlogPhoto(ctx, image.photo_id), image, options);
}

export function renderPostPhotos(ctx, post) {
  const image = editorialImage(post.image_id);
  return `<div class="blog-model-photos">${postPhotos(post).map((photo) => renderPhoto(ctx, resolveBlogPhoto(ctx, photo.id), image, { inline: true, modelIds: photo.model_ids.filter((id) => post.model_ids.includes(id)) })).join('')}</div>`;
}

export function renderEditorialCredits(ctx) {
  const zh = ctx.locale === 'zh-Hans';
  return `<section id="editorial-illustrations"><h2>${zh ? '博客图片' : 'Blog imagery'}</h2><p>${zh ? '博客使用下方所列厂家和零售商的车型照片，以远程方式展示并保留原始来源链接。照片配置与文章对比车型不同时，图注会明确说明。小熊猫是 China Bikes 的插画吉祥物。' : 'The blog displays model photos from the manufacturers and retailers listed below, with links to their original sources. Captions identify any differences between the pictured build and the article’s comparison. The red panda is the China Bikes illustrated mascot.'}</p><ul>${blogPhotos.map((item) => { const photo = resolveBlogPhoto(ctx, item.id); return `<li>${escapeHtml(photo.name)} — <a href="${escapeAttr(photo.source.url)}" rel="noreferrer">${escapeHtml(photo.image.credit)}</a></li>`; }).join('')}</ul></section>`;
}

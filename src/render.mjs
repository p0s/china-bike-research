...
function candidateFactRows(entry) {
  const labels = {
    ...
    tire_clearance_conflict: 'Tire clearance conflict',
    marketplace_material_reference: 'Marketplace material reference',
    marketplace_drivetrain_reference: 'Marketplace drivetrain reference',
    marketplace_reference_basis: 'Marketplace reference basis',
...
export function renderHome(ctx) {
  ...
  return page(ctx, {
    ...
    imageAlt: 'AI-generated wind-tunnel editorial scene with one iridescent aero bike on a test rig and six unbranded bikes behind glass; not exact-model photography',
    imageWidth: 1200,
    imageHeight: 630,
    imageType: 'image/png',
...
export function renderPrivacy(ctx) {
  const html = `<h2>Site behavior</h2><p>There are no accounts, advertising trackers, newsletters, or payment systems. Bike comparisons, the optional frameset allowance and the Build-page configuration stay in the visitor’s browser and may also be encoded in the URL when a visitor chooses or copies a build. Native-app telemetry is unchanged.</p><h2>Optional page analytics</h2><p>On the production site, eligible successful public HTML page requests are counted to understand request volume and estimate sessions. The estimate is approximate and does not claim to identify people or accurately measure human visits, downloads, conversions, or purchases. Assets, redirects, errors, prefetches, prerenders, recognizable bots, private paths, and this page’s preference actions are excluded.</p><p>The server-side collector receives the production hostname, public path, referring origin, trusted Cloudflare client IP, original browser user-agent, and an optional country code from hosting metadata. The IP and user-agent are processed transiently for device and session derivation, then discarded by the gateway. Live page-request data and country are retained for 13 months for this site’s analytics property only; encrypted operational backup copies expire within 30 days after live removal. There are no tracking cookies, cross-site identifiers, account links, query or fragment collection, screen-size fields, city, region, page titles, or event properties.</p><p>Do-not-track and Global Privacy Control are honored automatically. You can also set a host-only preference cookie; it is used only to remember this choice.</p><div class="privacy-choice"><form method="post" action="${url(ctx.base, '/analytics/opt-out')}"><button class="primary-button" type="submit">Opt out of page analytics</button></form><form method="post" action="${url(ctx.base, '/analytics/opt-in')}"><button class="text-button" type="submit">Opt in again</button></form></div><p>These forms use a same-origin POST and do not require JavaScript.</p><h2>Product images</h2><p>Some product photos load from their credited manufacturer or retailer, which receives a normal image request. Selected XHS and marketplace evidence images may load from the project’s separate media origin or from the same Cloudflare static site as small sanitized WebP derivatives. Public source links are reduced to identity-safe canonical post or listing URLs. Remote images use <code>referrerpolicy="no-referrer"</code>; when a source fails, the image is hidden and the product facts remain available.</p><h2>Optional videos</h2><p>Model pages do not contact YouTube when they first load. A video request is made to YouTube’s privacy-enhanced <code>youtube-nocookie.com</code> embed only after the visitor presses “Load video”; videos do not autoplay. The separate “Watch on YouTube” link opens YouTube directly.</p><h2>Public contributions</h2><p>GitHub issues and pull requests are public. Remove names, account details, addresses, order IDs, payment information, faces, license plates, location metadata, and share or referral parameters before submitting screenshots, photos, or source links. A removal request may identify the model and canonical source URL without publishing private contact details.</p>`;
  return prosePage(ctx, { title: 'Privacy', desc: 'Optional server-side page analytics and third-party media are disclosed.', path: '/privacy/', html });
}

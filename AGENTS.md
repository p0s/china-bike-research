...
- Keep only identity-safe canonical public URLs and privacy-safe summaries. Strip share, referral, invite, tracking, session, and account parameters. Third-party raw/original captures, media, and unreviewed binaries never enter Git. For this Cloudflare migration only, the bounded optimized derivatives listed in `docs/media-cloudflare-import-2026-09-22.md` may enter Git when each remains hash-, provenance-, rights-, attribution-, removal-route-, alt-text-, fallback-, and privacy-review-bound under `SPEC.md`; this does not grant broader research redistribution authority. Selected XHS or marketplace images may use `public-post-quotation` on the approved external media origin when each contributes exact-model or decision-relevant evidence and its record has the exact source, owner or seller credit, rights status in metadata, removal route, content hashes, alt text, fallback, and completed privacy review.
...
- **Images:** third-party raw/original captures, files, and unreviewed binaries never enter Git. The Cloudflare migration may commit only the reviewed optimized derivatives listed in `docs/media-cloudflare-import-2026-09-22.md`, each hash-linked to its source record and satisfying the exact `SPEC.md` provenance, rights, attribution, removal-route, alt-text, fallback, and privacy contract; this does not grant broader research redistribution authority. Preserve immutable originals and hash-linked OCR outside the repository. Direct remote references remain the default; the approved external media origin may serve selected attributed XHS or marketplace evidence images under `SPEC.md`. Preserve exactness, identity-safe canonical source, credit, alt text, privacy review, immutable derivative metadata, and fallback. Keep rights status in metadata; do not show legalistic filler such as “no license asserted” in the public UI.
